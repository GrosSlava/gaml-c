...
std::string FCompilerHelperLibrary::GetBestFolderName(const std::string& Path, const std::vector<std::string>& FoldersBestOrder)
{
	...
	{
		if( std::filesystem::exists(Path + (char)std::filesystem::path::preferred_separator + LFolderName) )
		{
			return LFolderName;
		}
	}
...
void FCompilerHelperLibrary::ReadAllFileToStr(std::ifstream& File, std::string& OutFileSource)
{
	OutFileSource.clear();
