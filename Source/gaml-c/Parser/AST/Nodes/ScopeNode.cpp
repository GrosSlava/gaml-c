...
public:
	...
	std::weak_ptr<UExprParseListNode> Left;
	std::weak_ptr<UExprParseListNode> Right;
};
...
public:
	...
	{
		std::shared_ptr<UExprParseListNode> LLeftListNode = ExprParseListNode->Left.lock();
		if( !LLeftListNode )
		{
			return;
		}

		if( std::shared_ptr<UExprParseListNode> LLeftLeftOperandListNode = LLeftListNode->Left.lock() )
		{
	...
	{
		std::shared_ptr<UExprParseListNode> LRightListNode = ExprParseListNode->Right.lock();
		if( !LRightListNode )
		{
			return;
		}

		if( std::shared_ptr<UExprParseListNode> LRightRightOperandListNode = LRightListNode->Right.lock() )
		{
...
std::shared_ptr<IASTNode> ScopeNode::ParseExpr(const std::vector<Token>& InTokens) const
{
	...
	while( LIndex < InTokens.size() )
	{
		...
		case ETokenType::LPAR:
		{
			...
			{
				FErrorLogger::Raise(EErrorMessageType::EXPECTED_RPAR, InTokens[LIndex], CurrentCompileOptions);
				return nullptr;
			}
...
#define CASE_OPERATOR(TokenName, ClassName)                                                                 \
	...
	{                                                                                                       \
		...
		{                                                                                                   \
			FErrorLogger::Raise(EErrorMessageType::INVALID_STATE, InTokens[LIndex], CurrentCompileOptions); \
			return nullptr;                                                                                 \
		}                                                                                                   \
...
#define CASE_KEYWORD_OPERATOR(TokenName, ClassName)                                                                                     \
	...
	{                                                                                                                                   \
		...
		if( LIndex + 1 >= InTokens.size() )                                                                                             \
		{                                                                                                                               \
			FErrorLogger::Raise(EErrorMessageType::EXPECTED_LPAR, InTokens[LIndex], CurrentCompileOptions);                             \
			return nullptr;                                                                                                             \
		}                                                                                                                               \
		++LIndex;                                                                                                                       \
...
#undef CASE_KEYWORD_OPERATOR
		...
		FErrorLogger::Raise(EErrorMessageType::UNEXPECTED_EXPRESSION, InTokens[LIndex], CurrentCompileOptions);
		return nullptr;
	}
	...
	for( long long int i = OperatorsPrioritySort.size() - 1; i >= 0; --i )
	{
		for( std::shared_ptr<UExprParseListNode> LExprParseListNode : OperatorsPrioritySort[i] )
		{
			...
			// clang-format on
			{
				...
				if( std::shared_ptr<UExprParseListNode> LLeftOperandListNode = LExprParseListNode->Left.lock() )
				{
					LLeftOperand = LLeftOperandListNode->ASTPart;
				}
				if( std::shared_ptr<UExprParseListNode> LRightOperandListNode = LExprParseListNode->Right.lock() )
				{
			...
			{
				FErrorLogger::Raise(EErrorMessageType::INVALID_STATE, InTokens[0], CurrentCompileOptions);
				return nullptr;
			}
		}
	}

	if( LASTParts.Head == nullptr || LASTParts.Head->Left.lock() != nullptr || LASTParts.Head->Right.lock() != nullptr )
	{
		FErrorLogger::Raise(EErrorMessageType::INVALID_STATE, InTokens[0], CurrentCompileOptions);
		return nullptr;
	}
...
std::shared_ptr<IASTNode> ScopeNode::Parse_Scope(const std::vector<Token>& InTokens, size_t& Index) const
{
	...
	{
		LScopeNode->BuildAST(std::vector<Token>(InTokens.begin() + Index + 1, InTokens.begin() + LBraScopeEnd), CurrentCompileOptions);
	}
